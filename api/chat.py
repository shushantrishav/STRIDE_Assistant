...
def verify_session(credentials: HTTPAuthorizationCredentials = Depends(security)):
    try:
        payload = jwt.decode(credentials.credentials, JWT_SECRET, algorithms=[JWT_ALGO], issuer=ISSUER)
...
def map_order_row(row):
    ...
    return {k: row[k] for k in ["order_id", "product_id", "size", "customer_id", "outlet_id", "purchase_date", "price", "category", "customer_phone"]}
...
def start_chat(request: Request, payload: ChatStartRequest):
    try:
        ...
        if not order or normalize_phone(payload.phone) != normalize_phone(order["customer_phone"]):
            logger.info(f"Invalid order/phone attempt: {payload.order_id}, {payload.phone}")
        ...
        prompt = request.app.state.prompt_builder.build_final_prompt(
            ...
            decision={"decision": "NEEDS_CLARIFICATION", "action": "ASK_CLARIFICATION", "generate_ticket": False, "ticket_type": None, "reason": ""},
            turn_count=1,
        )
        ...
        logger.info(f"Chat started for order {order['order_id']}, conversation {conversation_id}")

        return {"session_token": token, "message": ai_message, "turn": 1, "signals": result.get("signals")}
    except Exception as e:
...
# ---------------------------------------------------
@router.post("/respond")
def respond_chat(request: Request, payload: ChatRespondRequest, session=Depends(verify_session)):
    try:
        if session["turn"] != 2:
        ...
        if existing_ticket:
            prompt = request.app.state.prompt_builder.build_final_prompt(
                user_query=payload.message,
                decision={"decision": "EXISTING_TICKET", "action": "SHOW_EXISTING", "generate_ticket": False, "ticket_type": existing_ticket["ticket_type"], "reason": f"Ticket exists ({existing_ticket['ticket_id']})"},
                turn_count=2,
            )
            ai_message = run_llm(request, prompt)
        ...
        save_conversation_summary_with_ticket(
            ...
            reason=reason,
            order_ctx={**order, "days_since_purchase": (datetime.utcnow().date() - order["purchase_date"]).days, "warranty_days": 180, "notes": reason, "decision": final_ticket}
        )

        prompt = request.app.state.prompt_builder.build_final_prompt(
            user_query=payload.message,
            decision={"decision": final_ticket, "action": "TICKET_GENERATED", "generate_ticket": True, "ticket_type": final_ticket, "reason": reason},
            turn_count=2,
        )
