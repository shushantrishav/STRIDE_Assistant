...
def map_order_row(row):
    if not row:
        return None
    columns = [
        "order_id", "product_id", "size", "customer_id",
        "outlet_id", "purchase_date", "price", "category",
...
async def run_llm_generator(request: Request, user_query: str, sys_prompt: str) -> AsyncGenerator[str, None]:
    """
...
# ---------------------------------------------------
@router.post("/auth")
def chat_auth(payload: ChatAuthRequest):
    ...
    if not order or normalize_phone(payload.phone) != normalize_phone(order["customer_phone"]):
        ...
        raise HTTPException(401, "Invalid Order ID or Phone")

    conversation_id = create_conversation(payload.order_id)
    token = create_chat_token(payload.order_id, conversation_id)

    logger.info(f"Chat auth success | order={payload.order_id} convo={conversation_id}")
...
# SSE STREAM HELPER
# ---------------------------------------------------
async def sse_stream(request: Request, user_query: str, sys_prompt: str, conversation_id: str):
    """
...
async def start_chat(
    ...
    if get_ticket_by_order(order_id):
        msg = (f"Hello {c_name}, our records indicate that a support ticket is already "
               f"active for Order ID {order_id}. Please rest assured that our team is "
               "reviewing your case and will provide an update within 7 business days.")
        return {"message": msg, "conversation_completed": True}
...
async def respond_chat(
    ...
    save_conversation_summary_with_ticket(
        ...
        order_ctx={
            **order,
            "days_since_purchase": (
                date.today() - datetime.strptime(order["purchase_date"], "%Y-%m-%d").date()
            ).days,
            "warranty_days": 180,
            "notes": reason,
            "decision": final_ticket,
    ...
    return StreamingResponse(
        sse_stream(request, payload.message, final_result["llm_prompt"], conversation_id),
        media_type="text/event-stream",
        headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
    )
